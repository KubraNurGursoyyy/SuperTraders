...
const create = async (req, res) => {
    try {
        let _b = req.body;
        const created = await models.QuantityOfSharesInPortfolio.create(_b);
        if(created)
            return res.status(httpStatus.CREATED).send(respObject(0,'QuantityOfSharesInPortfolio created successfully.'));
        else
...
const remove = async (req, res) => {
    try {
        const  deleted = await models.QuantityOfSharesInPortfolio.destroy({
